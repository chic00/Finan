          ...
          {alerts.map((b) => (
            <div key={b.id} className="flex items-start gap-3 px-4 py-3 rounded-xl"
              style={{
                ...
                border: `1px solid ${b.isOverBudget
          ...
          return (
            <div key={budget.id} className="rounded-2xl p-6 transition-all hover:scale-[1.01]"
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className="font-medium" style={{ color: budget.isOverBudget ? 'var(--color-destructive)' : 'var(--color-foreground)' }}>
                  {budget.isOverBudget
        ...
        <Modal title={`Editar orçamento — ${editingBudget.category.name}`} onClose={() => setEditingBudget(null)}>
          <form onSubmit={handleEdit} className="space-y-4">
            ...
            <div className="px-4 py-3 rounded-xl text-sm"
              ...
              {MONTHS[editingBudget.month - 1]} {editingBudget.year} — já gasto: <strong style={{ color: 'var(--color-foreground)' }}>{formatCurrency(editingBudget.spent)}</strong>
            </div>
            <div>
              <label className="block text-sm font-medium mb-1.5" style={{ color: 'var(--color-foreground)' }}>Novo valor do orçamento</label>
              <input type="number" step="0.01" min="0.01" value={editAmount || ''} onChange={(e) => setEditAmount(parseFloat(e.target.value) || 0)}
                className="w-full px-4 py-2.5 rounded-xl outline-none transition-all theme-input"
